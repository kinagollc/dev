...
{		
	...
	public function actionIndex()
	{
		...
		if ($credentials = PayumoneyWrapper::getCredentials($merchant_id)){ 					
			...
			try {
				...
				$error = Yii::t("singleapp","Caught exception: [error]",array(
				  '[error]'=>$e->getMessage()
				));
			}    
			
		} else $error=st("invalid payment credentials");
	...
	public function actionverify()
	{
        ...
        $retHashSeq = $additionalCharges.'|'.$salt.'|'.$status.'|||||||||||'.$email.'|'.$firstname.'|'.$productinfo.'|'.$amount.'|'.$txnid.'|'.$key;
			...
			} else $error = st("Failed getting order information");			
		} else $error = st("invalid reference_id");		
