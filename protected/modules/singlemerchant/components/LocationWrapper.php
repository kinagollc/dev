...
class LocationWrapper
{
	...
	public static function GetStateList($page=0, $page_limit=10, $search_string='')
	{
		...
		if($country_id>0){
			...
			if($res=$db->rst($stmt)){
				...
				foreach ($res as $val) {														
				    $val['state'] = SingleAppClass::highlight_word( clearString($val['state_raw']) ,$search_string);
				    $data[] = $val;
				}
		...
		$and="";
   	    ...
   	    if($res = $db->rst($stmt)){
			...
			foreach ($res as $key=>$val) {
				...
				$val['city_name'] = SingleAppClass::highlight_word( clearString($val['name']) ,$search_string);
				$data[] = $val;
			}
	...
	public static function GetAreaList($city_id='',$page=0, $page_limit=10, $search_string='')
	{
		...
		if($res = $db->rst($stmt)){			
			...
			$data = array();
			foreach ($res as $key=>$val) {
				$val['area_name_raw'] = clearString($val['name']);
				$val['city_name'] = clearString($val['city_name']);
				$val['area_name'] = SingleAppClass::highlight_word( clearString($val['name']) ,$search_string);
				$data[] = $val;
			}								
			return array(
			  'paginate_total'=>$paginate_total,
	...
	public static function GetPostalCodeList($page=0, $page_limit=10, $search_string='')
	{
		...
		if($country_id>0){			
			...
			if($res=$db->rst($stmt)){
				...
				foreach ($res as $val) {														
				    $val['postal_code'] = SingleAppClass::highlight_word( clearString($val['postal_code_raw']) ,$search_string);
				    $data[] = $val;
				}
