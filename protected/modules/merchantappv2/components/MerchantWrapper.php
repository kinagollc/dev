...
class MerchantWrapper
{
	...
	public static function dashboardMenu()
	{
		...
		$data[] = array(
		 ...
		 'icon'=>"10.png",
		 'label'=>translate("Fee"),
		 'page'=>"shipping_list_location.html",
		 'access_id'=>'shippingrate'
		);	
		endif;
	...
	public static function getOptionsSettings()
	{
		...
		$name = array(
		 ...
		 'merchantapp_remove_cancel_status','merchant_disabled_registration'
		);
    ...
    {    	
    	...
    	if (Yii::app()->functions->isMerchantExist($params['contact_email'])){
    		throw new Exception( "Sorry you input email address that is already registered in our records." );
    	}
    	
    	if (Yii::app()->functions->validateUsername($params['username']) ){    		
    		throw new Exception( "Sorry but your username is alread been taken." );
    	}
    	
    	if ($respck=Yii::app()->functions->validateMerchantUserFromMerchantUser($params['username'],$params['contact_email'])){
		    throw new Exception($respck);
        }		    
    	...
    	$token = md5(FunctionsV3::generateCode(20));
    	unset($params['confirm_password']);
    	$params['activation_token'] = $token;
    	$params['activation_key'] = Yii::app()->functions->generateRandomKey(5);
    	$params['restaurant_slug'] = Yii::app()->functions->createSlug($params['restaurant_name']);    	
    	$params['status']=$status;
    	    	
    	if(Yii::app()->db->createCommand()->insert("{{merchant}}",$params)){
	        ...
	        //AUTO ADD SIZE
			FunctionsV3::autoAddSize($merchant_id);		 
			
			$cuisine = isset($params['cuisine'])?json_decode($params['cuisine'],true):array();
    ...
    public static function getMerchantByActivation($activation_token='')
    {
    	$stmt="
    	SELECT merchant_id,restaurant_name,payment_steps,
    	package_id,package_price,membership_expired,activation_key,contact_email,status,payment_gateway_ref
    	FROM {{merchant}}
    	WHERE activation_token=".q($activation_token)."
    	";
    	if($res = Yii::app()->db->createCommand($stmt)->queryRow()){
    		return $res;
    	}
