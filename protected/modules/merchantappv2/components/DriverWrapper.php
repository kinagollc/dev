	...
	{		
		...
		if($res = Yii::app()->db->createCommand($stmt)->queryRow()){	
			...
			if(method_exists("FunctionsV3","replaceTags")){
				...
				if(method_exists("MerchantWrapper","replaceTags")){
					$push_title = MerchantWrapper::replaceTags($push_title,$res);
				    $push_content = MerchantWrapper::replaceTags($push_content,$res);
				}
			}		
