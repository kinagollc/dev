...
class UpdateController extends CController
{
	...
	public function actionIndex()
	{
		...
		try {
			...
			/*1.0.8*/
			$loger[] = DatataseMigration::addColumn("{{item}}",array(
				'delivery_options'=>"text",
			));	
			
			$loger[] = DatataseMigration::addColumn("{{order}}",array(
		...
		$tbl_3 = Yii::app()->db->schema->getTable("{{driver_team}}");
		if($tbl_1 && $tbl_2 && $tbl_3){
			$stmt="
