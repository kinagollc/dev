...
{	
	...
	private function appSettings()
	{
		...
		$data['merchant_type_list'] = Merchant_utility::merchantTypeList();
		$data['invoice_terms_list'] = FunctionsV3::InvoiceTerms();
		$data['web_modules_api'] = websiteUrl()."/".APP_FOLDER;
	...
	public function actionAddItem()
	{
		...
		try {			
            ...
            ItemClass::insertMeta( 
              (integer)$merchant_id,
              (integer)$new_item_id,'delivery_vehicle',
	...
	{	
		...
		if($resp = FoodItemWrapper::getItem($merchant_id,$id)){									
			$data = array(
			  ...
			  'item_description_trans'=>isset($resp['item_description_trans'])?json_decode($resp['item_description_trans'],true):array(),		  
			  'delivery_options'=>isset($resp['delivery_options'])?json_decode($resp['delivery_options'],true):array(),
			);							
			$this->code = 1;
			$this->msg = "OK";
			$this->details = array(
		...
		if ($resp = OrderWrapper::getAllOrder($new,$order_type,$merchant_id,$page,$page_limit,$search_string)){			
			...
			foreach ($resp as $val) {	
				...
				CASE 
				    ...
				    (select CONCAT_WS('|',remarks,remarks2,remarks_args) from {{order_history}} 
				    where order_id=a.order_id and status='cancelled' limit 0,1  )
				...
				if($task_res = Yii::app()->db->createCommand($stmt_task)->queryRow()){						
					...
					if(!empty($task_res['task_remarks'])){
	...
	public function actionInvoiceGetByID()
	{
		...
		if($res = Yii::app()->db->createCommand($stmt)->queryRow()){			
			...
			if(empty($res['pdf_filename'])){
				$this->msg = translate("PDF not yet available");
				$this->output();
			}		
	...
	public function actionShippingLocationList()
	{
		$merchant_id = $this->validateToken();
		...
		if ($resp = LocationsWrapper::deliveryList($merchant_id,$page,$page_limit,$search_string)){				
			
			$data = array();$x=0;
			foreach ($resp as $val) {				
				$data[] = array(
				  'id'=>$val['rate_id'],
				  'name'=>Merchant_utility::formatNumber($val['fee']),
				  'description'=>translate("[state_name] [city_name] [area_name] [postal_code]",array(
				    '[state_name]'=>stripslashes($val['state_name']),
				    '[city_name]'=>stripslashes($val['city_name']),
				    '[area_name]'=>stripslashes($val['area_name']),
				    '[postal_code]'=>stripslashes($val['postal_code']),
				  )),
				  'thumbnail'=>FoodItemWrapper::getImage('','distance.png'),		
				  'status'=>translate("Min. Order [minimum_order]",array(
				   '[minimum_order]'=>Merchant_utility::formatNumber($val['minimum_order'])
				  )),
				  'date_created'=>translate("Free delivery above sub total [free_above_subtotal]",array(
				    '[free_above_subtotal]'=>Merchant_utility::formatNumber($val['free_above_subtotal'])
				  ))
				);				
				$x++;
			}
	...
	{
		$form_next = isset($this->data['form_next'])?$this->data['form_next']:'';		
		$country_id=getOptionA('location_default_country');
		$country = FunctionsV3::countryList(); $data = array();
		$this->code = 1;
		$this->msg = "OK";
		$this->details = array(
		 'data'=>$country,
		 'next_action'=>'store_country',	
		 'location_default_country'=>$country_id,
		 'form_next'=>$form_next
		);
		$this->output();
	}
		
	public function actionLocationStateList()
	{
		$data = array(); 
		$page_limit = MerchantWrapper::paginateLimit();
		$refresh  = isset($this->data['refresh'])?(integer)$this->data['refresh']:0;
		...
		if (isset($this->data['page'])){
        	$page = (integer)$this->data['page'] * $page_limit;
        } else  $page = 0;  
		
		$field_reference = isset($this->data['field_reference'])?(integer)$this->data['field_reference']:0;
							
		if ( $res = LocationsWrapper::StateList($field_reference,$page,$page_limit)){
			foreach ($res as $val) {
				$data[]=array(
				  'id'=>$val['state_id'],
				  'value'=>$val['name'],
	...
	public function actionLocationCityList()
	{
	    $data = array();
	    $page_limit = MerchantWrapper::paginateLimit();
		$refresh  = isset($this->data['refresh'])?(integer)$this->data['refresh']:0;
		...
		if ( $res = LocationsWrapper::CityList($field_reference,$page,$page_limit)){						
			foreach ($res as $val) {
				$data[]=array(
				  'id'=>$val['city_id'],
				  'value'=>$val['name'],
	...
	public function actionLocationAreaList()
	{
		$data = array();
		$page_limit = MerchantWrapper::paginateLimit();
		$refresh  = isset($this->data['refresh'])?(integer)$this->data['refresh']:0;
		...
		if ( $res = LocationsWrapper::AreaList($field_reference,$page,$page_limit)){						
			foreach ($res as $val) {
				$data[]=array(
				  'id'=>$val['area_id'],
				  'value'=>$val['name'],
	...
	public function actionAddShippingLocation()
	{		
		$merchant_id = $this->validateToken();
		$id = isset($this->data['id'])?(integer)$this->data['id']:0;					
		$params = array(
		  ...
		  'fee'=>isset($this->data['fee'])?(float)$this->data['fee']:0,
		...
		try {
			...
			LocationsWrapper::insertRate($merchant_id,$params,$id);
			$this->code = 1;
			$this->msg = $id>0?translate("Succesfully updated"):translate("Successful");
			$this->details = array(
			  'next_action'=>'pop_form'			  
	...
	public function actionShippingLocationGetByID()
	{		
		$merchant_id = $this->validateToken();
		$id = isset($this->data['id'])?(integer)$this->data['id']:0;		
		try {
						
			$resp = LocationsWrapper::getRate($id);
		    		
			$data = array(
			  'id'=>(integer)$resp['rate_id'],
			  'country_id'=>(integer)$resp['country_id'],
			  'state_id'=>(integer)$resp['state_id'],
			  'state_name'=>$resp['state_name'],
			  'city_id'=>(integer)$resp['city_id'],
			  'city_name'=>$resp['city_name'],
			  'area_id'=>(integer)$resp['area_id'],
			  'area_name'=>$resp['area_name'],
			  'fee'=>(float)$resp['fee'],
			  'minimum_order'=>(float)$resp['minimum_order'],
			  'free_above_subtotal'=>(float)$resp['free_above_subtotal'],
			);	
			...
			$this->details = array(
			  'next_action'=>'fill_form',
			  'form_id'=>"shipping_location_form.html",
			  'data'=>$data
			);			
		} catch (Exception $e) {			
			$this->msg = translate($e->getMessage());			
	...
	public function actionShippingLocationDelete()
	{
		$merchant_id = $this->validateToken();		
		$id = isset($this->data['id'])?(integer)$this->data['id']:0;		
		try {
									
			LocationsWrapper::deleteRate( $merchant_id,  $id );
	...
	public function actionSignup()
	{
		try {
			...
			$params = array(
			  'restaurant_name'=>isset($this->data['restaurant_name'])?addslashes($this->data['restaurant_name']):'',
			  'restaurant_phone'=>isset($this->data['restaurant_phone'])?addslashes($this->data['restaurant_phone']):'',
			  'contact_phone'=>isset($this->data['contact_phone'])?addslashes($this->data['contact_phone']):'',
			  'contact_email'=>isset($this->data['contact_email'])?addslashes($this->data['contact_email']):'',
			  'street'=>isset($this->data['street'])?addslashes($this->data['street']):'',
			  'city'=>isset($this->data['city'])?addslashes($this->data['city']):'',
			  'state'=>isset($this->data['state'])?addslashes($this->data['state']):'',
			  'post_code'=>isset($this->data['post_code'])?addslashes($this->data['post_code']):'',
			  'country_id'=>isset($this->data['country_id'])?addslashes($this->data['country_id']):'',
			  'service'=>isset($this->data['service'])?addslashes($this->data['service']):'',
			  'delivery_distance_covered'=>isset($this->data['delivery_distance_covered'])?(float)$this->data['delivery_distance_covered']:'',
			  'distance_unit'=>isset($this->data['distance_unit'])?addslashes($this->data['distance_unit']):'',
			  'username'=>isset($this->data['username'])?addslashes($this->data['username']):'',
			  'password'=>isset($this->data['password'])?md5($this->data['password']):'',
			  'confirm_password'=>isset($this->data['confirm_password'])?md5($this->data['confirm_password']):'',
			  'merchant_type'=>isset($this->data['merchant_type'])?(integer)($this->data['merchant_type']):'',
			  'cuisine'=>isset($this->data['cuisine'])?json_encode($this->data['cuisine']):'',
			);
			...
			$Validator->email(
			 array(
			   'contact_email'=>translate("Invalid email address")
			 ),$this->data
			);
			if(!$Validator->validate()){				
				$this->msg = translate("Invalid email address");
				$this->output();
			}		
			
			$message_1 = translate('Thank you!'); 
            $message_2 = translate('Please wait while our administrator validated your request, You will receive email once your merchant has been approved. Thank You.');
			
			if($params['merchant_type']==3){
				$params['invoice_terms'] = isset($this->data['invoice_terms'])?(integer)$this->data['invoice_terms']:0;
			}		
					
			if($params['merchant_type']==1){
				$next_action = "select_package";			
			} else {
				$next_action = "signup_thankyou";	
			...
			$resp = MerchantWrapper::signup($params);
			$this->code = 1;
			$this->msg = translate("Registration successful");
			$this->details = array(
			  'next_action'=>$next_action,
			  'activation_token'=>$resp,
			  'message_1'=>translate($message_1),
			  'message_2'=>translate($message_2),
			);
	...
	public function actionsignup_merchantinfo()
	{		
		try {
			
			$message_1 = translate("Thank you!");
			$message_2 = translate("Congratulation your merchant is now ready.");
			
			$next_action='';
			$activation_token = isset($this->data['activation_token'])?$this->data['activation_token']:'';
			$resp = MerchantWrapper::getMerchantByActivation($activation_token);
								
			if($resp['payment_steps']<=1){
				$next_action = "select_package";
			} else if ($resp['payment_steps']==2) {
				$next_action = "select_package";
			} else if ($resp['payment_steps']>=3) {
				if($resp['status']=="pending"){
					$email_verification = getOptionA('merchant_email_verification');
					...
					} else $next_action = "signup_verification";					
				} else $next_action = "signup_thankyou";				
			}
			...
			$this->msg = translate($e->getMessage());			
			$this->details = array(
			  'next_action'=>'clear_activation_token',
			);
		}		
		$this->output();
	}
	...
	public function actionpackageList()
	{
		try {
			...
			$data = array();
			$resp = Merchant_utility::planList();
			foreach ($resp as $val) {
				$val['title']=stripslashes($val['title']);
				$val['price']=FunctionsV3::prettyPrice($val['price']);
				$val['promo_price']=FunctionsV3::prettyPrice($val['promo_price']);
				$data[]=$val;
			}
	...
	public function actionSetPackage()
	{
		...
		try {
			...
			$package = Merchant_utility::getPackagesById($package_id);						
			...
			$this->msg = "OK";
			$this->details = array(
			  'next_action'=>'signup_payment_list',
			  'form_next'=>"signup_payment_list.html",
			  'activation_token'=>$activation_token
			);		
	...
	{		
		$activation_token = isset($this->data['activation_token'])?$this->data['activation_token']:'';
		$activation_key = isset($this->data['activation_key'])?$this->data['activation_key']:'';
		try {
			
			$resp = MerchantWrapper::getMerchantByActivation($activation_token);
			$merchant_id = $resp['merchant_id'];
					
			if($resp['activation_key']==$activation_key){
				
				Yii::app()->db->createCommand()->update("{{merchant}}",array(
				  'status'=>"active",			  
				  'date_modified'=>FunctionsV3::dateNow(),			 
				  'date_activated'=>FunctionsV3::dateNow(),
				  'ip_address'=>$_SERVER['REMOTE_ADDR']
				),
		  	    'merchant_id=:merchant_id',
			  	    array(
	...
	public function actionrefresh_outgoing()
	{
		$order_ids = '';
		$merchant_id = $this->validateToken();		
		MerchantWrapper::setMerchantTimezone($merchant_id);
		...
		if(!empty($order_ids)){
			$stmt="
			SELECT id,order_id FROM {{order_history}}
			WHERE order_id IN ($order_ids)
			AND date_created BETWEEN ".q($date_now)." - INTERVAL 1 MINUTE AND ".q($date_now)."
			";						
			if($res = Yii::app()->db->createCommand($stmt)->queryRow()){
				$need_refresh = true;
			} 
		...
		if($need_refresh){
		   $this->code = 1;		   
		   $this->details = array(
		     'next_action'=>"refresh_outgoing_tab",
		     'datenow'=>date('c')
		   );
		} else {
		   $this->details = array(
	...
	public function actionTransportType()
	{
		$order_ids = '';
		$merchant_id = $this->validateToken();		
		...
		$data = array();
		if(Merchant_utility::fileExist("components/Item_utility.php")){
		    $transport_type = Item_utility::transportType();		   		   
