...
class Driver
{
	...
	{		
		...
		if ( $res=$db->rst($stmt)){
			foreach ($res as $val) {
				...
				if(method_exists('Driver','consumeUrl')){
	    		   Driver::consumeUrl(FunctionsV3::getHostURL().Yii::app()->createUrl("driver/cron/autoassign"));	    		
				}
	   ...
	   $driver_status='active' , $team_id='', $driver_name='')
	{
		...
		$offline_interval_date = date("Y-m-d H:i:s", strtotime("-11 minutes"));
   ...
   public static function availableLanguages()
    {
    	$resp = FunctionsV3::getLanguageList(false);
    	return $resp;
    }   
	...
	{
		if(!Yii::app()->db->schema->getTable("{{driver_order_view}}")){	
			return false;
		}
		$db=new DbExt; 
		$db->qry("SET SQL_BIG_SELECTS=1");
		...
		if ( $key_is_enabled==1 && $info['order_id']>0){			
			...
			if ( FunctionsV3::hasModuleAddon('singlemerchant')){
				if ($single_resp = self::getSingleAppDeviceIDByOrderID($info['order_id'])){					
					$params_single=array(
					  'client_id'=>(integer)$single_resp['client_id'],
					  'client_name'=>isset($single_resp['client_name'])?$single_resp['client_name']:'',
					  'device_platform'=>isset($single_resp['device_platform'])?$single_resp['device_platform']:'',
					  'device_id'=>isset($single_resp['device_id'])?$single_resp['device_id']:'',
					  'push_title'=>!empty($push_title)?$push_title:'',
					  'push_message'=>!empty($message)?$message:'',
					  'date_created'=>FunctionsV3::dateNow(),
					  'ip_address'=>$_SERVER['REMOTE_ADDR'],
					  'merchant_id'=>isset($single_resp['merchant_id'])?(integer)$single_resp['merchant_id']:0,
					);																			
	...
	{		
		...
		if (is_numeric($order_id) && !empty($status)){			
			...
			Yii::app()->db->createCommand()->update("{{order}}",$params,
	  	    'order_id=:order_id',
		  	    array(
	  	    ...
	  	    /*NOTIFY MERCHANTAPP V2*/	  	    
	  	    if (FunctionsV3::hasModuleAddon("merchantappv2")){		    
		    	Yii::app()->setImport(array(			
				  'application.modules.merchantappv2.components.*',
			    ));
			    OrderWrapper::InsertOrderTrigger($order_id,'driver_update_to_merchant',$status,'driver_update_to_merchant');
			}		   
   ...
   public static function driverDropDownList($user_type='', $user_id='')
	...
	public static function backTrackList($user_type='', $user_id='')
	{
		...
		AND
		user_id=".self::q($user_id)."
		GROUP BY date_log,driver_id,date_created
		ORDER BY date_created DESC
		";		
		if ( $res=$db->rst($stmt)){
			return $res;
	...
	public static function BackgroundLocationMessage()
	{
		$message = "This app collects location data to enabled tracking your trips to work and calculated distance travelled even when the app is closed or not in use.";
				 
		$data = array();
		$data = array(
		  'title'=>Yii::t("driver","Allow [title] to access this device location?",array(
		   '[title]'=>getOptionA('website_title')
		  )),
		  'content'=>Yii::t("driver",$message,array(
		    '[title]'=>getOptionA('website_title'),
		    '[website]'=>websiteUrl()
		  ))
		);		
