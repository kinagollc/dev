	...
	if ( data.length >0) {
		$.each( data , function( index, val ) {
			...
			if (!empty(val.lat)){
		        ...
		        icon = mapbox_getIcon( val.map_type, val.trans_type, val.status_raw , val.is_online);
...
mapbox_getIcon = function(map_type, trans_type , status_raw, is_online){
	...
	if ( map_type=="restaurant"){
		...
		if(is_online==2){
			marker_icon = driver_icon_offline;
		} else {
