...
{	
    ...
    {    	
    	...
    	$background_message = Driver::BackgroundLocationMessage();
    	...
    	$this->details=array(
    	  ...
    	  'background_message'=>array(
    	    'title'=>isset($background_message['title'])?$background_message['title']:'',
    	    'content'=>isset($background_message['content'])?$background_message['content']:''
    	  ),
    	  'privacy_policy_link'=>getOptionA('driver_privacy_policy_link')
    	);
    ...
    {    	
    	...
    	//Yii::app()->functions->createLogs("test",'UpdateDriverLocation');
    	
    	$track_type = isset($this->data['track_type'])?$this->data['track_type']:'';
    	
    	/*PROVIDER EITHER LT OR mauron85*/
    	...
    	if(empty($track_type)){
	        ...
	        if(!empty($json)){                   
	            ...
	            if($provider=="geolocation-lt"){
	            	$this->data = $json;
	            } else $this->data = $json[0];	            
	        }        
    	}
    	    	       
        $this->data['token'] = isset($this->data['token'])?$this->data['token']:''; 
        
        /*CHANGE THE DATA WHEN PROVIDER IS LT*/
        if($provider=="geolocation-lt"){
        	$location_data = isset($this->data['location'])?$this->data['location']:'';        	
        	$this->data['lat'] = isset($location_data['coords']['latitude'])?$location_data['coords']['latitude']:'';
        	$this->data['lng'] = isset($location_data['coords']['longitude'])?$location_data['coords']['longitude']:'';
        	$this->data['altitude'] = isset($location_data['coords']['altitude'])?$location_data['coords']['altitude']:'';
        	$this->data['altitudeAccuracy'] = isset($location_data['coords']['altitude_accuracy'])?$location_data['coords']['altitude_accuracy']:'';
        	$this->data['heading'] = isset($location_data['coords']['heading'])?$location_data['coords']['heading']:'';
        	$this->data['speed'] = isset($location_data['coords']['speed'])?$location_data['coords']['speed']:'';
        }
    	...
    	if ($up){
    		...
    		$on_duty = $token['on_duty'];
    		
    		if ($is_record==1 && $on_duty ==1){
	    		$logs=array(	    		  
