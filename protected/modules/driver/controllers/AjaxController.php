...
class AjaxController extends CController
{
	...
	public function actionassignTask()
	{
		...
		if($Validator->validate()){
			...
			/*if($driverinfo = Driver::driverInfo($this->data['driver_id'])){
			   if($driverinfo['on_duty']!=1){
			   	  $this->msg = Driver::t("Driver is off duty");
			   	  $this->jsonResponse();
			   }
			}*/
	...
	{		
		...
		if (isset($this->data['id'])){
			if ( $res=Driver::getTaskId($this->data['id'])){
				...
				//if($info=$res){
				if($res){
					if($history_details = Driver::getTaskHistory($this->data['id'],$res['order_id'])){
	...
	{		
	    ...
	    Yii::app()->functions->updateOptionAdmin("driver_privacy_policy_link",
	    isset($this->data['driver_privacy_policy_link'])?$this->data['driver_privacy_policy_link']:'');	    
